...
void token::create() {
    ...
    auto sym = symbol("TSTT", 4);
	auto maximum_supply = asset(10000000, sym); //precision was defined as 4, so adding 4 zeros at the end of the quantity
...
void token::issue( const asset &quantity, const string &memo ) {
	...
	auto testsym_code = symbol("TSTT", 4); // TSTT is the token symbol with precision 4
    check( sym.code() == testsym_code.code(), "This contract can handle TSTT tokens only.");
	check( sym.is_valid(), "invalid symbol name" );
...
void token::transfer( const name& from, const name& to, const asset& quantity, const string& memo ) {
    ...
    auto testsym_code = symbol("TSTT", 4); // TSTT is the token symbol with precision 4
    check(sym == testsym_code.code(), "This contract can handle TSTT tokens only.");
...
void token::airgrab(const name &owner) {
     check(starts_with_vowel(owner.to_string()) == 0,
           "Account is not qualified, it must start with a, b, c or d.");
     check(_self != owner, "Cannot airgrab from TSTT owner account.");

     require_auth(owner);
     require_recipient(_self); // from
     require_recipient(owner); // to

     auto sym = symbol("TSTT", 4); // TSTT is the token symbol with precision 4
     asset airgrabbed_asset(100000, sym);    // allow 10 tokens to be airgrabbed
  ...
  int token::starts_with_vowel(string account_name) {
     if (!(account_name[0] == 'A' || account_name[0] == 'a' 
	 || account_name[0] == 'B' || account_name[0] == 'b' 
	 || account_name[0] == 'C' || account_name[0] == 'c'
     || account_name[0] == 'D' || account_name[0] == 'd'))
        return 1;
     else
        return 0;
  }
